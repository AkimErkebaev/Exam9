...
class ProfileView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        albums = user.albums.filter(is_private=False)
        photos = user.photos.filter(is_private=False)

        if self.request.user == user:
            private_photos = user.photos.filter(is_private=True)
            private_albums = user.albums.filter(is_private=True)

            context['private_photos'] = private_photos
            context['private_albums'] = private_albums

        favorite_photos = user.profile.favorites_photos.all()
        favorite_albums = user.profile.favorites_albums.all()
